...
trafic_2021 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2021.csv", sep = ";", header = T)
trafic_2021 <- trafic_2021 %>% 
  mutate(Année = "2021")

trafic_2020 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2020.csv", sep = ";", header = T)
trafic_2020 <- trafic_2020 %>% 
  mutate(Année = "2020")

trafic_2019 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2019.csv", sep = ";", header = T)
trafic_2019 <- trafic_2019 %>% 
  mutate(Année = "2019")

trafic_2018 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2018.csv", sep = ";", header = T)
trafic_2018 <- trafic_2018 %>% 
  mutate(Année = "2018")

trafic_2017 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2017.csv", sep = ";", header = T)
trafic_2017 <- trafic_2017 %>% 
  mutate(Année = "2017")

trafic_2016 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2016.csv", sep = ";", header = T)
trafic_2016 <- trafic_2016 %>%
  select(-12:-ncol(trafic_2016))
trafic_2016 <- trafic_2016 %>% 
  mutate(Année = "2016")

trafic_2015 <- read.csv("trafic-annuel-entrant-par-station-du-reseau-ferre-2015.csv", sep = ";", header = T)
...
trafic_boxplot <- trafic_boxplot %>%
  mutate(Couleur = case_when(
    Correspondance == "1" ~ "#FFCE00",
    Correspondance == "2" ~ "#0064B0",
    Correspondance == "3" ~ "#9F9825",
    Correspondance == "3bis" ~ "#98D4E2",
    Correspondance == "4" ~ "#C04191",
    Correspondance == "5" ~ "#F28E42",
    Correspondance == "6" ~ "#83C491",
    Correspondance == "7" ~ "#F3A4BA",
    Correspondance == "7bis" ~ "#83C491",
    Correspondance == "8" ~ "#CEADD2",
    Correspondance == "9" ~ "#D5C900",
    Correspondance == "10" ~ "#E3B32A",
    Correspondance == "11" ~ "#8D5E2A",
    Correspondance == "12" ~ "#00814F",
    Correspondance == "13" ~ "#98D4E2",
    Correspondance == "14" ~ "#662483",
    TRUE ~ Correspondance
  ))
...
geojson_data <- st_read("traces-du-reseau-ferre-idf.geojson")
geojson_data <- geojson_data %>%
  select(c(-1:-3, -5, -7:-21))
geojson_data <- geojson_data[geojson_data$mode == "METRO", ]

geojson_data2 <- read.csv("metro-france.csv", sep = ";", header = T)
